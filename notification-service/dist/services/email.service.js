...
exports.sendFriendRequestEmail = sendFriendRequestEmail;
exports.sendMeetingInvitationEmail = sendMeetingInvitationEmail;
exports.sendRecordingReadyEmail = sendRecordingReadyEmail;
...
async function sendFriendRequestEmail(data) {
    const content = `
    <div class="content">
  ...
  `;
    try {
        await email_1.emailTransporter.sendMail({
            from: `"Talk2 Notifications" <${FROM_EMAIL}>`,
            to: data.receiverEmail,
            subject: `${data.senderName} te envió una solicitud de amistad`,
            html: getEmailTemplate(content),
        });
        logger_1.logger.info(`✅ Friend request email sent to ${data.receiverEmail}`);
        return true;
    }
    catch (error) {
        logger_1.logger.error('❌ Error sending friend request email:', error);
        return false;
    }
}
// Meeting Invitation Email
async function sendMeetingInvitationEmail(data) {
  ...
  `;
    try {
        ...
        logger_1.logger.info(`✅ Meeting invitation email sent to ${data.invitedUserEmail}`);
        return true;
    }
    catch (error) {
        logger_1.logger.error('❌ Error sending meeting invitation email:', error);
        return false;
    }
}
// Recording Ready Email
async function sendRecordingReadyEmail(data) {
  ...
  `;
    try {
        ...
        logger_1.logger.info(`✅ Recording ready email sent to ${data.userEmail}`);
        return true;
    }
    catch (error) {
        logger_1.logger.error('❌ Error sending recording ready email:', error);
        return false;
    }
}
